            ...
            )
{
    ...
    size_t mt_brightness = data->mt_brightness;
    size_t mt_contrast = data->mt_contrast;
    ...
    for (; position < end; position += step) {
        ...
        pixels[position] =
            (uint8_t) UTILS_CLAMP(pixels[position] * mt_contrast + mt_brightness, 0.0f, 255.0f);
        pixels[position + 1] =
            (uint8_t) UTILS_CLAMP(pixels[position + 1] * mt_contrast + mt_brightness, 0.0f, 255.0f);
        pixels[position + 2] =
            (uint8_t) UTILS_CLAMP(pixels[position + 2] * mt_contrast + mt_brightness, 0.0f, 255.0f);

#elif defined SIMD_INTRINSICS_IMPLEMENTATION

        __m512 bri = _mm512_broadcastss_ps(_mm_load_ps(&mt_brightness));
        __m512 cont = _mm512_broadcastss_ps(_mm_load_ps(&mt_contrast));
        _m512i ints = _mm512_cvtepu8_epi32(_mm_load_si128((_m128i*) &pixels[position]));
        __m512 floats =_mm512_cvtepi32_ps(ints);
        floats = _mm512_fmadd_ps(floats, cont, bri);
        ...
        _asm_ _volatile_ (
                ...
                "vpmovusdb %%zmm0, (%2,%3)\n\t"
            ::
                "S"(&mt_brightness), "D"(&mt_contrast), "b"(pixels), "c"(position)
            :
                "%zmm0", "%zmm1", "%zmm2"
            );
...
int main(int argc, char *argv[])
{
    ...
    if (argc < 3) {
        fprintf(stderr, "Usage: %s <mt_brightness> <mt_contrast> <source file> <dest. file>\n", argv[0]);
        return result;
    }
    
    float mt_brightness = strtof(argv[1], NULL);
    float mt_contrast = strtof(argv[2], NULL);
    ...
    /* Main Image Processing Loop */
    {
        ...
        for (size_t position = 0; position < channels_count; position += channels_per_thread) {
            ...
            task_data->mt_brightness = mt_brightness;
            task_data->mt_contrast = mt_contrast;
